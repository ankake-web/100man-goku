...
## 更新（白枠の除去・進歩カードのフレーム表示）
- **白い四角の背景を除去**: 新しめのAI生成画像（騎士/建築/商品/カード/コマ等 計32枚）に、白い角丸の枠が
  焼き込まれていた（透明背景に見えても暗背景で白枠が出る）。エッジからのフラッドフィルで「透明＋白枠」を
  除去し図柄のみ残した（資源の羊毛＝本来白なので対象外、装飾フレームは枠なので別処理）。盤面・図鑑で白四角が消える。
- **進歩カードをフレーム表示**: 装飾フレーム `frame-decorative.png` の中央白面を透明化して「額縁」化し、
  カード効果モーダルで 絵をフレームで囲み＋カード名＋効果＋「使う/やめる」を表示（カードらしく・効果が分かる）。
