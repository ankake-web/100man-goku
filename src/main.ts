...
// 貴重品(紙/着物/金)取得アニメ用の画像。資源と同じく取得時に飛ばす。
...
function buildRulePanel(): HTMLDetailsElement {
  ...
  body.appendChild(ruleSection('⚔ 城下と武将（上級者向け・13点で勝ち）', [
    ...
    'この盤面では野盗は資源だけでなく貴重品も奪い、手札の上限判定（7の捨て札）も資源＋貴重品の合計で数える。',
  ]));
...
// 資源/貴重品の分配アニメ中フラグ。CPUの次手（特に次のダイス）を待たせ、サイコロと飛行アニメの被りを防ぐ。
...
function scheduleAiTurn(): void {
  ...
  if (state.phase === 'MAIN' && state.turnPhase === 'DISCARD') {
    // 捨て札済み(discardedThisRound)の AI は除外。判定は discardCount（武将と商いは資源＋貴重品）で。
...
function safeFallbackAction(): Action | null {
  ...
  // 武将と商いでは資源＋貴重品で数える（資源のみだと貴重品込みのCPUが選ばれず捨て札が終わらない）。
  if (state.phase === 'MAIN' && state.turnPhase === 'DISCARD') {
...
function spawnResFlyer(
  imgSrc: string,                     // 飛ばすアイコン画像（資源 or 貴重品）
  ...
  setTimeout(() => {
    ...
    if (hidden) {
      ...
      // 手札カードと同じ画像で飛ばす（資源・貴重品とも。絵文字→画像で見た目を統一）。
      const img = document.createElement('img');
...
// 武将と商いでは貴重品も奪取され得るので合算する（強奪アニメ判定用）。
...
// 公開情報の資源/貴重品の合計枚数（他者はマスクで handCount/commodityCount、自分は実手札）。
...
// 貴重品(紙/着物/金)の手札差分（増分のみ）。非ダイスの取得（交易等）で自分の増分を飛ばす用。
...
// 貴重品 c を「このダイス目で産出し、かつ pid の城に隣接する」タイルの画面中心を返す。
// 貴重品は城からのみ産出（森林=紙/牧=着物/鉱山=金）。見つからなければ該当地形タイル→盤面中央へ。
...
function triggerResourceAnimation(
  ...
  // 武将と商いでは「資源＋貴重品(紙/着物/金)」を城産出に合わせて飛ばす（資源と同じ演出）。
  // LANでは相手の手札がマスクされ手札差分が0になるため、この公開導出が必須。
  // ダイス以外（交易・豊作等）は手札差分（自分のみ公開）で資源・貴重品とも飛ばす。
  ...
  const MAX_PER_PLAYER = 8; // スマホで重くならないよう1人あたりの上限（貴重品分を見込み少し増やす）
  ...
  for (const pid of newState.playerOrder) {
    // 飛ばすアイコン群（資源・貴重品を統一して扱う）。hidden=種類非公開（札裏トークン）。
    ...
    if (ckProd) {
      ...
      if (exact.length > 0) {
        ...
        // 資源・貴重品それぞれの増分を別々に見る。片方が減りもう片方が増える札
        // （例: 商業湊=資源を渡し貴重品を得る）でも、増えた側を伏せ札で見せる。
